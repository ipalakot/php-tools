...
/* FONCTION AVG  
		...
		try{
			...
			$foncsql= "SELECT AVG(Age) FROM inscrits";

			$requetes = $connexion-> prepare($foncsql);
			$requetes -> execute();

			$resultat = $requetes-> fetchall();
...
/* FONCTION COUNT/SUM/ AVG/ MAX/MIN 
		$serveur="localhost";
		$login="root";
		$pass="";
		...
		try{
			...
			$foncsql= "SELECT SUM(Age) FROM inscrits";
 ...
 /* FONCTION UCASE/LCASE  
		...
		try{
			...
			$foncsql= "SELECT ROUND(Dons, 1) FROM inscrits";
